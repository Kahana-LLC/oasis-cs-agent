...
The build script copies `baseline_snapshot.json` into `public/` and **syncs email HTML previews**: for each sequence- or touch-level `preview.source` in `email_sequences.json`, it wraps the Brevo fragment from `brevo-oasis-emails/` and writes `public/emails/<touch_id>.html` (or sequence id for legacy single-touch rows; gitignored; regenerated every deploy). It also writes **`public/emails/copy_manifest.json`** (raw HTML/plain text keyed by touch id for Copy buttons on `/email-machine`). Run `python reporting/build_static_site.py` locally after editing templates to refresh previews before spot-checking `/email-machine`.
